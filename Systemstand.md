...
Stand: **02.05.2026** · Codebasis am Projektroot `<projektpfad>/auftragssystem` (lokalen Klone hier einsetzen). Diese Datei beschreibt den **Ist-Zustand** (Tech-Stack, Supabase-Schema laut `src/types/supabase.ts`, Workflows und zentrale Logik). Keine Migrationsdateien im Repo — die Datenbank wird über Supabase gepflegt.
