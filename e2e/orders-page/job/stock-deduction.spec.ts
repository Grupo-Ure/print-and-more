...
test.describe('in progress, stamp job in pre-press, model in stock', () => {
  ...
  test('releasing the job to production deducts the product quantity from the model stock', async ({ ordersPage, navbar, stampStockPage, job }) => {
    // Setup — the stock the model is left with and its row on the stock page, with the job open.
  ...
  test('marking the job done after the release deducts nothing more', async ({ ordersPage, navbar, stampStockPage, job }) => {
    // Setup — the stock the model is left with and its row on the stock page, with the job open.
    const remaining = IN_STOCK_STAMP_MODEL.stock - IN_STOCK_STAMP_PRODUCT_ROW.quantity
    const modelRow = stampStockPage.table.row(IN_STOCK_STAMP_MODEL.id)
    await ordersPage.openJob(job)
...
test.describe('in progress, textile job in pre-press, variant in stock', () => {
  ...
  test('releasing the job to production deducts the garment quantity from the variant stock', async ({ ordersPage, navbar, textileStockPage, job }) => {
    // Setup — the stock the variant is left with and its row on the stock page, with the job open.
  ...
  test('marking the job done after the release deducts nothing more', async ({ ordersPage, navbar, textileStockPage, job }) => {
    // Setup — the stock the variant is left with and its row on the stock page, with the job open.
    const remaining = IN_STOCK_TEXTILE_CHAIN.variant.stock - OWN_STOCK_GARMENT_ROW.quantity
    const variantRow = textileStockPage.table.row(IN_STOCK_TEXTILE_CHAIN.variant.id)
    await ordersPage.openJob(job)
