...
/** The order or job has high priority. */
export function HighPriorityFlag({ size, testId }: FlagProps) {
  return (
    <span data-testid={testId} title="High priority" className="inline-flex shrink-0">
      <ArrowUp size={size} className="text-red-600 animate-bounce" aria-label="High priority" />
    </span>
  )
}
