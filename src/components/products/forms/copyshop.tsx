  ...
  return (
    <FormShell onSubmit={() => void form.handleSubmit()}>
      <form.Subscribe selector={s => s.values}>
        {values => {
          const errors = validateProduct('POSTER', values, p.subOrderStatus) as Errs
          return (
            <>
...
export function BrochureForm(p: ProductFormProps) {
  ...
  return (
    <FormShell onSubmit={() => void form.handleSubmit()}>
      <form.Subscribe selector={s => s.values}>
        {values => {
          const errors = validateProduct('BROCHURE', values, p.subOrderStatus) as Errs
          const pp = values.production_path
          const cc = (matKey: string, otherKey: string, label: string) => (
            <>
              <form.Field name={matKey}>{f => <SelectField field={f} label={label} options={CC_OPTS} error={errors[matKey]} />}</form.Field>
...
export function BusinessCardForm(p: ProductFormProps) {
  ...
  return (
    <FormShell onSubmit={() => void form.handleSubmit()}>
      <form.Subscribe selector={s => s.values}>
        {values => {
          const errors = validateProduct('BUSINESS_CARD', values, p.subOrderStatus) as Errs
          return (
            <>
...
export function BindingForm(p: ProductFormProps) {
  ...
  return (
    <FormShell onSubmit={() => void form.handleSubmit()}>
      <form.Subscribe selector={s => s.values}>
        {values => {
          const errors = validateProduct('BINDING', values, p.subOrderStatus) as Errs
...
export function PrintoutForm(p: ProductFormProps) {
  ...
  return (
    <FormShell onSubmit={() => void form.handleSubmit()}>
      <form.Subscribe selector={s => s.values}>
        {values => {
          const errors = validateProduct('PRINTOUT', values, p.subOrderStatus) as Errs
          return (
            <>
